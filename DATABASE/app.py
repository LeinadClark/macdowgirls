...
def suggest_campaign():
    ...
    try:
        ...
        db.session.commit()
        return jsonify({"status": "success", "message": "Thank you! Your suggestion has been submitted for review."}), 201
    except Exception as e:
        db.session.rollback()
...
def sign_petition():
    ...
    existing = PetitionSignature.query.filter_by(email=email).first()
    if existing:
        return jsonify({"status": "error", "message": "This email has already signed the petition."}), 400
